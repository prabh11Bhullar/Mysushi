...
//  Created by Satwinder Kaur on 2019-11-03.
...
class InterfaceController: WKInterfaceController, WCSessionDelegate {
    ...
    func session(_ session: WCSession, didReceiveMessage message: [String : Any]) {
        ...
        if (message.keys.contains("timeRemaining")){
            ...
            print("WATCH: Hey i Got message from Phone = \(messageBody)")
            
            
            if messageBody == 0 {
                self.lblTimeRemaining.setHidden(false)
                self.lblTimeRemaining.setText("OOPS...GAME OVER")
                self.btnMoveleft.setHidden(true)
                self.btnMoveRight.setHidden(true)
            }
            else {
                self.lblTimeRemaining.setHidden(false)
                self.lblTimeRemaining.setText("Hey\(messageBody) Seconds Remaining")
                
                //Hide time remaining notification after 2 seconds of recieving message
    ...
    @IBAction func gesturePause(_ sender: Any) {
        if (WCSession.default.isReachable) {
            ...
            print("WATCH: Sorry Cannot reach phone")
        }
    }
