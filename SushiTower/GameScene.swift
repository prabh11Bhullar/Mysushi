...
class GameScene: SKScene, WCSessionDelegate {
    ...
    let caty = SKSpriteNode(imageNamed: "character1")
    ...
    // Show life n score labels
    let lifeLabel = SKLabelNode(text:"LIVES: ")
    let scoreLabel = SKLabelNode(text:"SCORES: ")
    ...
    override func didMove(to view: SKView) {
        ...
        // add cat
        caty.position = CGPoint(x:self.size.width*0.25, y:100)
        addChild(caty)
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if SecondsRemaining == 0 {
            ...
            self.secondsRemainingLabel.text = "OOPS...GAME OVER"
        }
    ...
    public func moveCat(){
        ...
        if self.moveDirection == "left" {
            ...
            self.caty.run(facingRight)
            
            // save cat's position
            self.catPosition = "left"
        }
        if self.moveDirection == "right"{
            caty.position = CGPoint(x:self.size.width*0.85, y:100)
            
            // change the cat's direction
            let facingLeft = SKAction.scaleX(to: -1, duration: 0)
            self.caty.run(facingLeft)
        ...
        if (self.sushiTower.count > 0) {
            ...
            let chopstickPosition = firstSushi.positionOfStick
